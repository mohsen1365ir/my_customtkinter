...
class CTkCheckBox(tkinter.Frame):
    ...
    def __init__(self, *args,
        ...
        if self.textvariable is not None:
            ...
            #    self.select(from_variable_callback=False)
            # elif self.variable.get() == self.offvalue:
            #    self.deselect(from_variable_callback=False)
    ...
    def configure(self, *args, **kwargs):
        ...
        if "variable" in kwargs:
            ...
            if self.variable is not None and self.variable != "":
                self.variable_callback_name = self.variable.trace_add("write", self.variable_callback)
                if self.variable.get() == self.onvalue:
                    print('called_select from configure')
    ...
    def variable_callback(self, var_name, index, mode):
        if not self.variable_callback_blocked:
            if self.variable.get() == self.onvalue:
                print('called select from variable callback')
                self.select(from_variable_callback=True)
            elif self.variable.get() == self.offvalue:
                self.deselect(from_variable_callback=True)
